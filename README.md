...
| File | Description |
|------|-------------|
| [src/settings.local.json](src/settings.local.json) | Permission configuration with deny rules |
| [src/audit.sh](src/audit.sh) | Final script containing all denied commands and exfiltration |
...
- **GHSA-xq4m-mc3c-vvg3:** Parser failed to detect command execution via `$IFS` and short CLI flags. Accepted because the parser should have caught the embedded execution.
- **GHSA-x5gv-jw7f-j6xj:** Overly broad allowlist enabled file read and network exfiltration without confirmation. Accepted for the same impact class demonstrated here.
